...
public class App {
    ...
    private static void registe(ArrayList<User> list) {
       Scanner sc=new Scanner(System.in);
        String uername;
       while (true) {
           System.out.println("请输入用户名");
           uername = sc.next();
           boolean flag1 = checkUserName(uername);
           if (!flag1) {
               System.out.println("格式不满足条件，需要重新输入");
               continue;
           }
          boolean flag2=contains(list,uername);
           if(flag2){
               System.out.println("用户名"+uername+"已存在，请重新输入");
           }else {
               System.out.println("用户名"+uername+"可用");
               break;
           }
       }
        String password;
       while (true) {
            ...
            if(!password.equals(againPassword)){
                ...
                System.out.println("俩次密码一致，继续录入其他数据");
                break;
            }
        }
        String personID;
       ...
       User u=new User(uername,password,personID,phoneNumber);
       list.add(u);
    ...
    private static void  login(ArrayList<User> list) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (!flag) {
                System.out.println("用户名" + username + "未注册请先注册登录");
                return;
            }
            ...
            if (result) {
                System.out.println("登陆成功，可以开始使用学生系统");
                //创建对象调用方法，启动学生管理系统
